...
import { Client, Guild, Member, Message, Role, User } from "eris";
...
function waitForResponse(bot: Client, user: User, ...args: string[]): Promise<Message>{
	return new Promise((resolve) => {
		const responseFunc = (message: Message) => {
			if (message.author.id === user.id){
				resolve(message);
				bot.off("messageCreate", responseFunc);
			}
		};

		bot.on("messageCreate", responseFunc);
